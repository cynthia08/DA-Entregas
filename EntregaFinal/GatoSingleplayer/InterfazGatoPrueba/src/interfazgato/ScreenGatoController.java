...
public class ScreenGatoController implements Initializable, ControlScreen {
    ...
    private void handle(MouseEvent event) {
        ...
        img.addEventHandler(MouseEvent.MOUSE_CLICKED, new EventHandler<MouseEvent>() {
            ...
            public void handle(MouseEvent event) {
                ...
                if(controlador.posicionValida(tablero, casilla)) {
                    if (controlador.turno().equals("O")) {
                    ...
                    controlador.juego(tablero, casilla);
                }
            }
        });
    }
    ...
    public void anunciaGanador(boolean empate) {
        panelGanador.setVisible(true);
        rec.setVisible(true);
        
        if(empate) {
            labelGanador.setText("Empate. Gato.");
        } else {
            if(controlador.turno().equals("X")){
                labelGanador.setText("Ganador " + nomJugador1.getText() + " !!!");
                GatoHelper.ganador(LogHelper.getIdJug1());
            }
            else{
                labelGanador.setText("Ganador " + nomJugador2.getText() + " !!!");
                GatoHelper.ganador(LogHelper.getIdJug2());
            }
        }
    }
}
